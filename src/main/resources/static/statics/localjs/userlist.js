...
$('#addUserNow').click(function () {
    $.post("/backend/loadUserAndCodeTypeList.action", function(data) {
        if (data != "") {
            $.each(data, function (i, result) {
                if (i == 1) {
                    for (var i = 0; i < result.length; i++) {
                        $("#selectcardtype").append("<option value=\"" + result[i].valueId + "\">" + result[i].valueName + "</option>");
                    }
                }
            });
        } else {
...
$('.addusercancel').click(function(e){
	...
	$("#a_username").val('');
	$("#selectcardtype").val('');
	$("#selectcardtype").empty();
    $("#selectcardtype").html('<option value=\"\" selected=\"selected\">--请选择--</option>');
	$("#a_idcard").val('');
	$("#a_mobile").val('');
	$("#a_email").val('');
	$("#a_postCode").val('');
		...
		success: function(result){
			if("failed" == result){
				...
				if(roleId == '2'){
					...
					$.post("/backend/loadUserAndCodeTypeList.action",{'s_role':roleId},function(data){
						if (data != "") {
                            $.each(data, function (i, result) {
                                if (i == 0) {
                                    for (var i = 0; i < result.length; i++) {
...
$("#m_roleId").change(function(){
	...
	if(sel_role == 2){
		$.post("/backend/loadUserAndCodeTypeList.action",{'s_role':sel_role},function(data){
            if (data != "") {
                $.each(data, function (i, result) {
                    if (i == 0) {
                        for (var i = 0; i < result.length; i++) {
